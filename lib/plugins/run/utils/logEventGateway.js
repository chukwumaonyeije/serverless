...
const getMessage = msg => {
  ...
  if (parsedMsg.msg === 'Function registered.') {
    ...
    const response = colorDim(prettifyValue(JSON.parse(parsedMsg.response)));
    const responseText = `${response}`;
    const text = `Function invoked: ${parsedMsg.functionId} by event ${
      JSON.parse(parsedMsg.event).event
    }`;
    return `${text}${os.EOL}${responseText}`;
